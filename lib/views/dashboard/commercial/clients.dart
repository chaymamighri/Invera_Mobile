...
class _CommercialClientsSectionState extends State<CommercialClientsSection>
      ...
      child: Column(
        ...
        children: [
          ...
          if (_clientTypes.isNotEmpty) ...[
            SizedBox(height: isCompactPage ? _baseUnit : _baseUnit * 2),
            _buildTypeChips(isCompactPage: isCompactPage),
          ],
        ],
      ),
    );
  ...
  Widget _buildTypeChips({required bool isCompactPage}) {
    final chips = <Widget>[
      _buildTypeChip(
        label: 'Tous',
        selected: _selectedType == null,
        isCompactPage: isCompactPage,
        onSelected: () => _onSelectType(null),
      ),
      ..._clientTypes.map(
        (type) => _buildTypeChip(
    ...
    return Wrap(spacing: _baseUnit, runSpacing: _baseUnit, children: chips);
  }
  ...
  Widget _buildTypeChip({
    ...
    return ChoiceChip(
      label: Text(label),
      selected: selected,
      showCheckmark: !isCompactPage,
      materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
      visualDensity: isCompactPage
          ? const VisualDensity(horizontal: -4, vertical: -4)
          : VisualDensity.compact,
      labelPadding: EdgeInsets.symmetric(horizontal: isCompactPage ? 4 : 8),
      padding: EdgeInsets.symmetric(
        horizontal: isCompactPage ? 6 : 8,
        vertical: isCompactPage ? 2 : 6,
      ),
      ...
      side: BorderSide(
        color: selected ? _primary.withValues(alpha: 0.35) : _borderLight,
      ),
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(isCompactPage ? 12 : 999),
      ),
